...
# WebSocket endpoints — priority: env var > Alchemy (if key set) > Ankr (free, no signup)
def _get_wss(chain: str) -> str:
    env_var = os.environ.get(f"RPC_WSS_{chain.upper()}")
    if env_var:
        return env_var
    if ALCHEMY_API_KEY:
        if chain == "base":
            return f"wss://base-mainnet.g.alchemy.com/v2/{ALCHEMY_API_KEY}"
        return f"wss://eth-mainnet.g.alchemy.com/v2/{ALCHEMY_API_KEY}"
    # Ankr free public endpoints — no key needed
    if chain == "base":
        return "wss://rpc.ankr.com/base/ws"
    return "wss://rpc.ankr.com/eth/ws"

ALCHEMY_BASE_WS = _get_wss("base")
ALCHEMY_ETH_WS  = _get_wss("eth")
...
async def _monitor_chain(bot: Bot, chain: str):
    ...
    # Check if we have a real WSS URL (not just the Ankr fallback needing a key)
    if not ws_url or ws_url == "":
        logger.warning(f"[EVM_MON] No WSS URL for {chain} — monitor disabled.")
        return
    logger.info(f"[EVM_MON] Using WSS: {ws_url[:40]}...")
...
async def _send_evm_alert(bot: Bot, ca: str, chain: str, meta: dict, dev: dict, tx_hash: str, elapsed_ms: int):
    ...
    try:
        ...
        asyncio.ensure_future(evaluate_and_alert(bot, ca, chain))
    except Exception as e:
        logger.warning(f"[EVM_MON] sniper hook error: {e}")
