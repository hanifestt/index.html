...
async def _get_holders_from_transfers(session, ca, chain, total_supply):
    ...
    try:
        ...
        from_block   = hex(max(0, latest_block - 50000))  # last ~50000 blocks
...
async def get_evm_holders(session: aiohttp.ClientSession, ca: str, chain: str) -> dict:
    ...
    try:
        ...
        # Scan last 50000 blocks (~27 hours on Base, ~7 hours on ETH)
        from_block = hex(max(0, latest_block - 50000))
        transfer_topic = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
        ...
        logs = log_data.get("result", [])
        logger.info(f"[EVM] Holder logs found: {len(logs) if isinstance(logs, list) else logs}")
        if not logs:
            return _holder_defaults()

        # Get block timestamps for freshness check
        cutoff_blocks = latest_block - 43200  # ~24h on Base (2s blocks)
...
async def get_evm_dev_history(session: aiohttp.ClientSession, ca: str, chain: str) -> dict:
    ...
    try:
        ...
        # Method 1: trace the contract creation tx using eth_getTransactionReceipt
        # Get earliest block where contract exists by scanning creation via eth_getLogs
        # Use ContractCreated pattern — get tx that deployed this contract
        # eth_getStorageAt with slot 0 to find owner/deployer patterns
        try:
            # Get the block the contract was created in using binary search on eth_getCode
            latest_payload = {"jsonrpc": "2.0", "id": 10, "method": "eth_blockNumber", "params": []}
            async with session.post(rpc, json=latest_payload, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                latest_data = await resp.json()
            latest_block = int(latest_data.get("result", "0x0"), 16)

            # Scan recent blocks for the deployment tx using eth_getLogs with no topics
            # Contract creation shows as tx with empty 'to' field
            # Use debug_traceBlockByNumber if available, else trace via tx scan
            # Most reliable: get first Transfer event from this contract (mint tx)
            transfer_topic = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
            zero_addr = "0x" + "0" * 64  # zero address as topic

            # Get very first Transfer event (from zero = mint = deployment)
            from_block_hex = hex(max(0, latest_block - 500000))
            logs_payload = {
                "jsonrpc": "2.0", "id": 11,
                "method": "eth_getLogs",
                "params": [{
                    "address": ca,
                    "topics": [transfer_topic, zero_addr],
            ...
            mint_logs = logs_data.get("result", [])
            if mint_logs:
                # Get the tx hash of the first mint
                creation_tx = mint_logs[0].get("transactionHash", "")
                if creation_tx:
                    tx_payload = {
                        "jsonrpc": "2.0", "id": 12,
            ...
            logger.warning(f"[EVM] deployer trace error: {e}")
