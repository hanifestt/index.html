...
async def get_evm_supply(session, ca, chain):
    try:
        ...
        # Try progressively wider ranges until we find logs
        logs = []
        for block_range in [100000, 500000, 2000000, 5000000]:
            from_block = hex(max(0, latest - block_range))
            try:
                ...
                logs = result or []
                logger.info(f"[EVM] supply logs: {len(logs)} at range {block_range}")
                if logs:
                    break
            except Exception as e:
                logger.warning(f"[EVM] supply log range {block_range} error: {e}")
                continue
    ...
    try:
        ...
        for block_range in [100000, 500000, 2000000, 5000000]:
            ...
            try:
                ...
                logs = result or []
                logger.info(f"[EVM] holder logs: {len(logs)} at range {block_range}")
                if logs:
                    break
            except Exception as e:
                logger.warning(f"[EVM] holder log range {block_range} error: {e}")
                continue
    ...
    try:
        ...
        if not deployer:
            try:
                latest_hex = await rpc_call(session, chain, "eth_blockNumber", [])
                latest     = int(latest_hex, 16)
                # Try wide range for older tokens
                mint_logs = []
                for br in [500000, 2000000, 5000000, 10000000]:
                    from_block = hex(max(0, latest - br))
                    try:
                        result = await rpc_call(session, chain, "eth_getLogs", [{
                            "address": ca,
