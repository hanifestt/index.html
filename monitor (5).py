...
async def run_monitor(bot: Bot):
    ...
    while True:
        try:
            ...
            err_str = str(e)
            if "429" in err_str:
                _retry_count = getattr(run_monitor, "_retry_count", 0) + 1
                run_monitor._retry_count = _retry_count
                wait = min(60 * _retry_count, 600)  # 60s, 120s, 180s... max 10min
                logger.warning(f"[MONITOR] Rate limited (429). Retry #{_retry_count}, waiting {wait}s...")
            elif "401" in err_str:
                wait = 300
                logger.error(f"[MONITOR] Auth error (401). Check HELIUS_API_KEY. Waiting {wait}s...")
            else:
                run_monitor._retry_count = 0  # reset on non-429 error
                wait = 15
                logger.error(f"[MONITOR] Connection error: {e}. Reconnecting in {wait}s...")
            try:
