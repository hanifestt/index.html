...
async def scan_token(ca: str) -> dict:
    logger.info(f"[SCAN] Starting scan for {ca[:8]}... HELIUS_RPC={HELIUS_RPC[:40]}")
    async with aiohttp.ClientSession() as session:
        # Quick RPC health check
        try:
            test = {"jsonrpc": "2.0", "id": 1, "method": "getHealth", "params": []}
            async with session.post(HELIUS_RPC, json=test, timeout=aiohttp.ClientTimeout(total=5)) as r:
                health = await r.json()
            logger.info(f"[SCAN] Helius health: status={r.status} result={health.get('result','?')}")
        except Exception as e:
            logger.error(f"[SCAN] Helius unreachable: {e}")
    ...
    logger.info(f"[SCAN] Results — wallets={wallet_data} supply={supply_data}")
...
async def scan_wallets(session: aiohttp.ClientSession, ca: str) -> dict:
    """Use getTokenLargestAccounts (standard RPC, free tier) instead of
    getTokenAccounts (Helius-specific, rate limited on free plan)."""
    try:
        # Get top 100 holders via standard RPC
        payload = {
            "jsonrpc": "2.0", "id": 1,
        ...
        async with session.post(HELIUS_RPC, json=payload, timeout=aiohttp.ClientTimeout(total=12)) as resp:
            if resp.status != 200:
                logger.warning(f"[WALLETS] getTokenLargestAccounts status={resp.status}")
                return _wallet_defaults()
            data = await resp.json()

        accounts = data.get("result", {}).get("value", [])
        if not accounts:
            logger.warning("[WALLETS] no accounts returned")
            return _wallet_defaults()

        # Resolve token accounts to owner wallets
        addrs = [a["address"] for a in accounts if a.get("address")]
        if not addrs:
            return _wallet_defaults()
        ...
        owners = []
        for acc in mp_data.get("result", {}).get("value", []) or []:
            if acc:
                owner = acc.get("data", {}).get("parsed", {}).get("info", {}).get("owner", "")
                if owner:
                    owners.append(owner)

        wallet_count = len(set(owners))

        # Cluster detection: check if wallets share funding source
        # Quick check: look for wallets created in same block (coordinated)
        from collections import Counter
        owner_counts = Counter(owners)
        duplicates   = sum(1 for c in owner_counts.values() if c > 1)
        cluster_pct  = round(min(duplicates / max(wallet_count, 1) * 100, 100), 1)

        # Fresh wallet check: get signatures for a sample of owners
        fresh_count = 0
        ...
        for owner in sample:
            try:
                sig_payload = {
                    ...
                    "params": [owner, {"limit": 1, "commitment": "finalized"}]
                }
                async with session.post(HELIUS_RPC, json=sig_payload,
                                        timeout=aiohttp.ClientTimeout(total=4)) as resp:
                    sig_data = await resp.json()
                sigs = sig_data.get("result", [])
                # If wallet's OLDEST tx is recent, it's fresh
                if sigs:
                    oldest_tx_time = sigs[-1].get("blockTime", 0) or 0
                    if oldest_tx_time > cutoff:
                        fresh_count += 1
            except Exception:
                pass

        fresh_pct = round(fresh_count / max(len(sample), 1) * 100, 1)

        logger.info(f"[WALLETS] count={wallet_count} cluster={cluster_pct}% fresh={fresh_pct}%")
        return {
        ...
        logger.error(f"[WALLETS] error: {e}", exc_info=True)
        return _wallet_defaults()
...
# ── MEV scan — uses standard getSignaturesForAddress RPC ─────────────────────
async def scan_mev(session: aiohttp.ClientSession, ca: str) -> dict:
    try:
        # Get recent signatures for this token program address
        payload = {
            "jsonrpc": "2.0", "id": 1,
            "method": "getSignaturesForAddress",
            "params": [ca, {"limit": 50, "commitment": "finalized"}]
        }
        ...
        # Group by slot — same slot with multiple txs = possible MEV
        slot_map: dict = {}
        for s in sigs:
            slot = s.get("slot", 0)
            slot_map.setdefault(slot, []).append(s.get("signature", ""))

        # Sandwich pattern: 3+ txs in same slot
        sandwich_count = sum(1 for txs in slot_map.values() if len(txs) >= 3)
...
async def get_deployed_tokens(session: aiohttp.ClientSession, deployer: str) -> list:
    ...
    if len(token_mints) < 3:
        try:
            ...
            before = None
            for page in range(3):
                # Use standard getSignaturesForAddress instead of enhanced API
                sig_payload = {
                    "jsonrpc": "2.0", "id": 1,
                    "method": "getSignaturesForAddress",
                    "params": [deployer, {"limit": 100, "commitment": "finalized",
                ...
                oldest_ts = sigs[-1].get("blockTime", 0) if sigs else 0

                # For each sig, get full tx to find token mints
                for sig_info in sigs[:20]:  # limit to 20 to stay fast
                    sig = sig_info.get("signature", "")
                    ts  = sig_info.get("blockTime", 0) or 0
                    try:
                        tx_payload = {
                            "jsonrpc": "2.0", "id": 2,
                            "method": "getTransaction",
                        ...
                        for ix in (tx.get("transaction", {}).get("message", {})
                                   .get("instructions", []) or []):
                ...
                txs = sigs  # for loop continuation logic
                if not txs: break
